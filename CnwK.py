...
def calculate_schedule_pressure_rating(
    ...
    if days_to_deadline <= 0:
        ...
        return (
            5,
            "The deadline has already passed, or no delivery time remains, creating an immediate schedule risk.",
            scenarios,
        )
    ...
    if pressure <= 0.60:
        rating = 1
        reason = (
            f"Schedule pressure is low. The expected delivery time of "
            f"{scenarios['expected_days']} days remains comfortably within the available timeframe."
        )
    elif pressure <= 0.85:
        rating = 2
        reason = (
            f"Schedule pressure is manageable. The expected delivery time of "
            f"{scenarios['expected_days']} days is within the deadline, although flexibility is reduced."
        )
    elif pressure <= 1.00:
        rating = 3
        reason = (
            f"The schedule is tight. The expected delivery time of "
            f"{scenarios['expected_days']} days is close to the available deadline, leaving limited room for delays."
        )
    elif pressure <= 1.25:
        rating = 4
        reason = (
            f"The schedule is under significant pressure. The expected delivery time of "
            f"{scenarios['expected_days']} days is likely to exceed the available timeframe."
        )
    else:
        rating = 5
        reason = (
            f"The schedule is under severe pressure. The expected delivery time of "
            f"{scenarios['expected_days']} days substantially exceeds the available deadline."
        )
...
def calculate_expertise_rating(expertise: int) -> Tuple[int, str]:
    ...
    mapping = {
        1: (
            5,
            "The team has very limited relevant expertise, which greatly increases the likelihood of delays, errors, or delivery issues.",
        ),
        2: (
            4,
            "The team has low relevant expertise, which increases the chance of delivery problems and reliance on additional support.",
        ),
        3: (
            3,
            "The team has moderate expertise, so delivery appears feasible but some capability risk remains.",
        ),
        4: (
            2,
            "The team has good expertise, which reduces the likelihood of delivery issues.",
        ),
        5: (
            1,
            "The team has strong expertise, which significantly reduces the likelihood of delivery problems.",
        ),
    }
    return mapping.get(
        expertise,
        (3, "The team has moderate expertise, so some capability risk remains."),
    )
...
def calculate_risk_score(
    *,
    ...
    probability_weights = normalize_weights(
        probability_weights or {},
        DEFAULT_PROBABILITY_WEIGHTS,
    )

    impact_weights = normalize_weights(
        impact_weights or {},
        DEFAULT_IMPACT_WEIGHTS,
    )
    ...
    probability_factors: Dict[str, Dict[str, object]] = {
        "expertise": {
            ...
            "weight": probability_weights["expertise"],
            "reason": expertise_reason,
        },
        "schedule_pressure": {
            "rating": schedule_rating,
            "weight": probability_weights["schedule_pressure"],
            "reason": schedule_reason,
        },
        "clarity": {
            "rating": clarity_rating,
            "weight": probability_weights["clarity"],
            "reason": clarity_reason,
        },
        "dependency": {
            "rating": dependency_rating,
            "weight": probability_weights["dependency"],
            "reason": dependency_reason,
        },
        "complexity": {
            "rating": complexity_rating,
            "weight": probability_weights["complexity"],
    ...
    impact_factors: Dict[str, Dict[str, object]] = {
        "budget": {
            ...
            "weight": impact_weights["budget"],
            "reason": budget_reason,
        },
        "complexity": {
            "rating": complexity_rating,
            "weight": impact_weights["complexity"],
            "reason": complexity_reason,
        },
        "clarity": {
            "rating": clarity_rating,
            "weight": impact_weights["clarity"],
            "reason": clarity_reason,
        },
        "dependency": {
            "rating": dependency_rating,
            "weight": impact_weights["dependency"],
            "reason": dependency_reason,
        },
        "schedule_impact": {
            "rating": schedule_rating,
            "weight": impact_weights["schedule_impact"],
            "reason": schedule_reason,
        },
    }
    ...
    return {
        ...
        "applied_probability_weights": probability_weights,
        "applied_impact_weights": impact_weights,
        "factor_ratings": {
